...
// 排他ロックする
func flockByTenantID(tenantID int64, lockType string) (io.Closer, error) {
...
func billingReportByCompetition(ctx context.Context, tenantDB dbOrTx, tenantID int64, competitonID string) (*BillingReport, error) {
	...
	fl, err := flockByTenantID(tenantID, "shared")
	if err != nil {
...
func competitionScoreHandler(c echo.Context) error {
	...
	fl, err := flockByTenantID(v.tenantID, "exclusive")
	if err != nil {
...
func playerHandler(c echo.Context) error {
	...
	// player_scoreを読んでいるときに更新が走ると不整合が起こるのでロックを取得する
	fl, err := flockByTenantID(v.tenantID, "shared")
	if err != nil {
		return fmt.Errorf("error flockByTenantID: %w", err)
	}
	defer fl.Close()
